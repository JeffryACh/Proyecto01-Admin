...
class MileageViewModel @Inject constructor(
    ...
    fun registerMileage() {
        ...
        viewModelScope.launch {
            ...
            val historyResult = mileageRepository.registerMileage(record)
            if (historyResult.isFailure) {
                _uiState.update { it.copy(isLoading = false, error = historyResult.exceptionOrNull()?.message ?: "No se pudo registrar el kilometraje") }
                return@launch
            }
            vehicleRepository.updateVehicleMileage(vehicle.id, km).fold(
                onSuccess = { _uiState.update { it.copy(vehicle = vehicle.copy(kilometraje_actual = km), currentMileageInput = "", isLoading = false, isSuccess = true) } },
                onFailure = { e -> _uiState.update { it.copy(isLoading = false, error = e.message ?: "Se guardó el historial, pero no se pudo actualizar el vehículo") } }
            )
        }
    }
}
